 ...
 */
const coordTransformation_1 = require("../coordTransformation");
const pathProcessor_1 = require("../pathProcessor");
const coordHelpers_1 = require("./coordHelpers");
const constants_1 = require("./constants");
const MM_PER_PIXEL = 50;
function buildRobotToPixel(image, scaleFactor, dimensionsAreScaled) {
    ...
    return (robotX, robotY) => (0, coordTransformation_1.robotToPixel)({
        x: robotX,
        y: robotY,
...
function getRoomName(segmentId, options) {
    const rn = options.roomNames;
    if (!rn)
        return undefined;
    if (rn instanceof Map)
        return rn.get(segmentId);
    return rn[segmentId];
}
/**
 ...
 */
async function drawMapV1(mapData, renderer, options = {}) {
    const scaleFactor = options.scaleFactor ?? constants_1.VISUAL_BLOCK_SIZE;
    ...
    const pixel = (px) => (0, coordHelpers_1.getPixelFromScaledDimensions)(scaledWidth, scaledHeight, px);
    // --- Floor + obstacle rects ---
    const floorRects = [];
    let bounds;
    const floorColor = (0, constants_1.hexToRgbaString)("#E9E9E9");
    const obstacleColor = (0, constants_1.hexToRgbaString)("#6B7174");
    if (image.pixels?.floor) {
        for (const px of image.pixels.floor) {
            const { x, y } = pixel(px);
            floorRects.push({ x, y, w: constants_1.VISUAL_BLOCK_SIZE, h: constants_1.VISUAL_BLOCK_SIZE, fill: floorColor });
            if (!bounds)
                bounds = { minX: x, minY: y, maxX: x + constants_1.VISUAL_BLOCK_SIZE, maxY: y + constants_1.VISUAL_BLOCK_SIZE };
            else {
                bounds.minX = Math.min(bounds.minX, x);
                bounds.minY = Math.min(bounds.minY, y);
                bounds.maxX = Math.max(bounds.maxX, x + constants_1.VISUAL_BLOCK_SIZE);
                bounds.maxY = Math.max(bounds.maxY, y + constants_1.VISUAL_BLOCK_SIZE);
            }
        }
    }
    if (image.pixels?.obstacle) {
        for (const px of image.pixels.obstacle) {
            const { x, y } = pixel(px);
            floorRects.push({ x, y, w: constants_1.VISUAL_BLOCK_SIZE, h: constants_1.VISUAL_BLOCK_SIZE, fill: obstacleColor });
            if (!bounds)
                bounds = { minX: x, minY: y, maxX: x + constants_1.VISUAL_BLOCK_SIZE, maxY: y + constants_1.VISUAL_BLOCK_SIZE };
            else {
                bounds.minX = Math.min(bounds.minX, x);
    ...
    if (image.pixels?.segments) {
        for (const px of image.pixels.segments) {
            ...
            if (!segmentsData[segnum])
                segmentsData[segnum] = { minX: x, maxX: x, minY: y, maxY: y };
            else {
            ...
            const fill = options.getSegmentColor ? options.getSegmentColor(segnum) : (0, constants_1.hexToRgbaString)("#CCCCCC");
            if (fill)
                segmentRects.push({ x, y, w: constants_1.VISUAL_BLOCK_SIZE, h: constants_1.VISUAL_BLOCK_SIZE, fill });
        }
    }
    renderer.drawSegmentRects(segmentRects);
    // Clean snapshot (backend only): after segments, before carpet
    if (renderer.getCleanSnapshot)
    ...
    if (!mopPath?.length && mapData.PATH?.points?.length)
        mopPath = new Array(mapData.PATH.points.length).fill(0);
    const pathResult = mapData.PATH?.points && mopPath
        ? (0, pathProcessor_1.processPaths)(mapData.PATH.points, mopPath, (robotPoint) => robotToPx(robotPoint[0], robotPoint[1]), scaleFactor, null)
        : {
            mainPath: [[]],
            backwashPath: [[]],
            pureCleanPath: [[]],
    ...
    const lwMain = Math.max(1, constants_1.VISUAL_BLOCK_SIZE / 2);
    const lwBackwash = constants_1.VISUAL_BLOCK_SIZE * 0.5;
    renderer.drawPath({
        segments: pathResult.mopPath,
        stroke: "rgba(255, 255, 255, 1)",
        lineWidth: 6.5 * constants_1.VISUAL_BLOCK_SIZE,
        opacity: 0.18,
        pathLayer: "mop",
    });
    renderer.drawPath({
        segments: pathResult.mainPath,
        stroke: constants_1.LEGACY_COLORS.path,
        lineWidth: lwMain,
        pathLayer: "main",
    });
    renderer.drawPath({
    ...
    if (mapData.VIRTUAL_WALLS?.length) {
        for (const wall of mapData.VIRTUAL_WALLS) {
            ...
            virtualWalls.push({
                ...
                lineWidth: 1 * constants_1.VISUAL_BLOCK_SIZE,
            });
        }
    }
    ...
    if (mapData.GOTO_PREDICTED_PATH?.points?.length) {
        ...
        renderer.drawPredictedPath({
            points,
            stroke: "rgba(255, 255, 255, 1)",
            lineWidth: (3 * constants_1.VISUAL_BLOCK_SIZE) / 2,
            dashArray: [3 * constants_1.VISUAL_BLOCK_SIZE, 3 * constants_1.VISUAL_BLOCK_SIZE],
        });
    }
    else {
    ...
    if (drawObs && typeof drawObs.then === "function")
        await drawObs;
    // --- Robot ---
    ...
    // --- Room labels ---
    const roomLabels = (options.roomLabels?.length ?? 0) > 0
        ? options.roomLabels
        : (() => {
            const out = [];
            for (const [segnumStr, bounds] of Object.entries(segmentsData)) {
                ...
                if (!name)
                    continue;
                const centerX = bounds.minX + (bounds.maxX - bounds.minX) / 2 + constants_1.VISUAL_BLOCK_SIZE / 2;
                const centerY = bounds.minY + (bounds.maxY - bounds.minY) / 2 + constants_1.VISUAL_BLOCK_SIZE / 2;
...
//# sourceMappingURL=drawMapV1.js.map