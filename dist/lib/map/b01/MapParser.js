...
class MapParser {
    ...
    parse(rawData, serial, model, duid, connectionType) {
        try {
            ...
            const payload = (0, B01MapPayloadClassifier_1.classifyB01MapPayload)(decrypted);
            ...
            this.adapter.rLog(connectionType, duid, "Warn", "B01", 301, "[MapParser] Unsupported B01 payload format after decryption.", "warn");
            return null;
        }
        catch (e) {
    ...
    parseProtobuf(buffer, duid, connectionType) {
        ...
        try {
            ...
            if (level === "warn") {
                this.adapter.rLog(connectionType, duid, "Warn", "B01", 301, `Protobuf decode failure: ${e.message}`, "warn");
            }
        }
        return mapData;
    }
