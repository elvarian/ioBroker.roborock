...
class MapBuilder {
    SCALE = 8; // High Res output
    assets = null;
    assetsLoaded = false;
    adapter; // Optional reference to adapter for logging
    offsetCache = new Map();
    constructor(adapter) {
        this.adapter = adapter;
    }
    /** @param needRobotCharger If false (e.g. history map), do not log errors for missing robot/charger assets. */
    async loadAssets(model, duid, needRobotCharger = true) {
        ...
        try {
            ...
            const findAssetInPaths = async (candidates) => {
            ...
            if (b01RobotImg)
                this.assets.robot["b01_fixed"] = b01RobotImg;
            ...
            if (!Object.keys(this.assets.robot).length) {
                ...
                if (fallback)
                    this.assets.robot["cleaning"] = fallback;
            }
            ...
            if (chargerImg)
                this.assets.charger["normal"] = chargerImg;
            ...
            // Also try the named ones from ROOM_TYPE_MAP (kitchen, bedroom, etc) IF they exist in some form
            for (const name of Object.values(constants_1.ROOM_TYPE_MAP)) {
                if (!name || name === "other")
                    continue;
                const candidates = [
            ...
            if (otherImg)
                this.assets.rooms["other"] = otherImg;
            ...
            if (this.adapter)
                this.adapter.rLog("MapManager", duid, "Error", undefined, undefined, `Error loading assets: ${e.message}`, "error");
        }
        this.assetsLoaded = true;
    }
    getRobotIconAssetKey(status) {
        ...
        if (deviceState === constants_1.SUBTITLE_STATUS.WAIT_INSTRUCTION || deviceState === constants_1.SUBTITLE_STATUS.IDEL) {
            return "icon_87";
        }
        if (deviceState === constants_1.SUBTITLE_STATUS.RECHARGING || deviceState === constants_1.SUBTITLE_STATUS.CHARGE_FULL || deviceState === constants_1.SUBTITLE_STATUS.BREAK_CHARGING) {
            return "icon_88";
        }
        // Fault handling
        ...
        if (deviceState === constants_1.SUBTITLE_STATUS.SLEEP) {
            return isDarkMode ? "icon_91" : "icon_92";
        }
        // WorkMode logic
        const isCleaning = [constants_1.JOB_STATUS.CLEANING, constants_1.JOB_STATUS.ZONED_CLEANING, constants_1.JOB_STATUS.SPOT_CLEANING].includes(deviceWorkMode);
        const isPause = deviceWorkMode === constants_1.JOB_STATUS.PAUSED;
        const isGoCharging = deviceWorkMode === 6; // RETURNING
        const isBuildModel = status.deviceCustomType === constants_1.CleanModeType.ALL; // Map custom type to icon
        if (isBuildModel && isCleaning) {
            key = "icon_93";
        }
        if (isGoCharging || deviceState === constants_1.SUBTITLE_STATUS.BREAK_RECHARGING) {
            key = isDarkMode ? "icon_94" : "icon_95";
        }
        // Additional operative state logic (Segment/Point modes)
        if (deviceState === constants_1.SUBTITLE_STATUS.PAUSE) {
        ...
        if (isCleaning) {
            if (deviceCleanMode === constants_1.SCCleanType.clean)
                key = "icon_102";
            if (deviceCleanMode === constants_1.SCCleanType.both)
                key = "icon_103";
            if (deviceCleanMode === constants_1.SCCleanType.mop)
                key = "icon_104";
        }
        if (this.adapter)
            this.adapter.rLog("MapManager", undefined, "Debug", undefined, undefined, `Selected Robot Icon: ${key} for state=${deviceState}, work=${deviceWorkMode}, clean=${deviceCleanMode}`, "debug");
        return key;
    }
    getVisibleOffset(img) {
        if (this.offsetCache.has(img))
            return this.offsetCache.get(img);
        const w = img.naturalWidth;
        const h = img.naturalHeight;
        const cnv = (0, canvas_1.createCanvas)(w, h);
    ...
    drawMapAsset(ctx, pos, phi, img, unitWidth, offsetX, offsetY, toPixel) {
    ...
    async buildMap(data, robotModel, duid, deviceStatus) {
        ...
        const canvas = (0, canvas_1.createCanvas)(width * this.SCALE, height * this.SCALE);
        ...
        if (data.rooms) {
            data.rooms.forEach(r => {
                if (r.colorId !== undefined)
                    roomColorMap[r.gridValue ?? r.roomId] = r.colorId;
            });
        }
        ...
        const COLOR_SET = constants_1.SC_MAP_COLORS.LEVEL_1;
        ...
        const wallColor = hexToBytes(constants_1.PALETTE.WALL);
        const floorColor = hexToBytes(constants_1.PALETTE.FLOOR);
        const unknownColor = hexToBytes(constants_1.PALETTE.UNKNOWN);
        ...
        for (let i = 0; i < data.mapGrid.length; i++) {
            ...
            if (val === 0)
                continue; // Skip transparency (default 0)
            const x = i % width;
        ...
        // Render Zones
        if (data.areasInfo)
            data.areasInfo.forEach(area => drawNoGoZone(area, false));
        ...
        if (data.recmForbitZone)
            data.recmForbitZone.forEach(zone => drawNoGoZone(zone, true));
        // Render Carpets
        if (data.carpetInfo) {
            data.carpetInfo.forEach(carpet => {
                if (carpet.points && carpet.points.length > 0) {
                    ...
                    ctx.fillStyle = constants_1.APP_COLORS.circleColor;
                    ctx.beginPath();
        ...
        if (data.history && data.history.length > 0) {
            ...
            points.forEach((p, idx) => {
                ...
                if (prevP) {
                    ...
                    if (dist > 0.5)
                        breakSegment = true; // Jump detection
                }
            ...
            const drawPathSegment = (seg, color, width, dash = []) => {
            ...
            segments.forEach(seg => {
                if (seg.type === "mopPaths" || seg.type === "mixPaths")
                    drawPathSegment(seg, "#FFFFFF66", 3.3);
            });
            // Pass 2: Foregrounds
            segments.forEach(seg => {
                if (seg.type === "solidPath")
                    drawPathSegment(seg, "rgba(255, 255, 255, 1)", 0.75);
                else if (seg.type === "mopPaths")
                    drawPathSegment(seg, "#FFFFFF99", 0.75);
                else if (seg.type === "mixPaths")
                    drawPathSegment(seg, "#FFFFFF", 0.75);
                else if (seg.type === "dottPath")
                    drawPathSegment(seg, "rgba(255, 255, 255, 0.6)", 0.45, [1, 2]);
            });
            ctx.setLineDash([]);
        }
        ...
        if (data.chargerPos) {
            ...
            if (chargerImg) {
                ...
                this.drawMapAsset(ctx, data.chargerPos, data.chargerPos.phi, chargerImg, drawW, 0, // Zero Offset
                0, // Zero Offset
                toPixel);
            }
            else {
        ...
        if (data.robotPos) {
            const status = deviceStatus || {
                deviceState: constants_1.SUBTITLE_STATUS.IDEL,
                deviceWorkMode: constants_1.JOB_STATUS.IDLE,
                deviceCleanMode: constants_1.SCCleanType.clean
            };
            ...
            if (robotImg) {
                ...
                this.drawMapAsset(ctx, data.robotPos, data.robotPos.phi || 0, robotImg, ROBOT_MAP_SIZE, 0, // Zero Offset
                0, // Zero Offset
                toPixel);
            }
            else {
        ...
        if (data.rooms) {
            ...
            data.rooms.forEach(r => {
                if (r.labelPos && r.roomName) {
                    ...
                    const finalType = constants_1.ROOM_TYPE_MAP[r.roomTypeId || 0] || "other";
                    // 2. Select Icon Asset
                    ...
                    const BG_COLOR_SET = constants_1.SC_MAP_COLORS.ROOM_ICON_BG;
                    const BORDER_COLOR_SET = constants_1.SC_MAP_COLORS.ROOM_ICON_BORDER;
                    ...
                    if (roomIcon) {
                        ...
                        ctx.drawImage(roomIcon, startX + (iconSize - imgSize) / 2, centerY - imgSize / 2, imgSize, imgSize);
                    }
    ...
    getRoomFillColorIndex(roomId, colorId) {
        if (colorId !== undefined) {
            return colorId > 0 ? colorId - 1 : 0;
        }
        return roomId > 0 ? roomId - 1 : 0;
    }
    getRoomLabelColorIndex(colorId) {
