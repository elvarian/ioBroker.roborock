...
exports.Q10MapCreator = void 0;
const B01StateSemantics_1 = require("../b01/B01StateSemantics");
const roomNameNormalizer_1 = require("../../roomNameNormalizer");
const Q10Verification_1 = require("./Q10Verification");
const ROOM_COLOR_COUNT = 4;
const ROOM_OTHER_MATERIAL = 3;
const Q10_DOCK_ANCHORED_OFFSET = 3.5;
function devicePointToPixel(source, point) {
    return {
...
function shouldAnchorRobotToDock(deviceStatus) {
    const stateCode = deviceStatus?.deviceState;
    if ((0, B01StateSemantics_1.isB01DockAnchoredState)(stateCode))
        return true;
...
function mapArrPointToPixel(point) {
    return {
        x: point.x,
        y: point.y
    };
}
function sourceAreaToCreator(source, area) {
    return {
        id: area.id,
        type: area.type,
...
function sourceLineToCreator(source, area) {
...
function mapSourceAreas(source, areas) {
    return areas.map((area) => sourceAreaToCreator(source, area));
}
function mapSourceLines(source, areas) {
    return areas
        .map((area) => sourceLineToCreator(source, area))
        .filter((area) => area !== null);
}
function isRoomValue(value) {
    return value > 1 && value < 127;
}
function analyzeRoomStatsFromGrid(mapGrid, width, height) {
    const stats = new Map();
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const value = mapGrid[y * width + x];
            if (!isRoomValue(value))
                continue;
...
function analyzeRoomStats(mapData, mapGrid = mapData.mapGrid) {
    return analyzeRoomStatsFromGrid(mapGrid, mapData.header.sizeX, mapData.header.sizeY);
}
function getLineInterpolation(start, end) {
...
function getPointMapValue(point, mapGrid, width, height) {
    ...
    if (index >= 0 && index < mapGrid.length)
        return mapGrid[index] ?? -9999;
    return -9999;
}
function getRoomCrossPoints(start, end, roomGridValue, mapGrid, width, height) {
    ...
    for (let index = 0; index < linePoints.length; index++) {
        ...
        if (index > 0 && index < linePoints.length - 1) {
            ...
            if (lastRoomValue === roomGridValue && nextRoomValue === roomGridValue)
                continue;
        }
...
function fixRoomCrossPoints(points) {
    if (points.length < 4)
        return points;
...
function computeRoomLabelCenter(mapGrid, mapWidth, mapHeight, roomGridValue, stat) {
    ...
    let widthCrossPoints = getRoomCrossPoints({ x: stat.minX - 1, y: centerY }, { x: stat.maxX + 1, y: centerY }, roomGridValue, mapGrid, mapWidth, mapHeight);
    ...
    let heightCrossPoints = getRoomCrossPoints({ x: centerX, y: stat.minY - 1 }, { x: centerX, y: stat.maxY + 1 }, roomGridValue, mapGrid, mapWidth, mapHeight);
    ...
    if (heightCrossPoints.length >= 2) {
        ...
        if (centerRoomValue !== roomGridValue) {
            ...
            for (let index = 0; index < heightCrossPoints.length / 2; index++) {
                if (index * 2 + 1 > heightCrossPoints.length - 1)
                    break;
...
function buildRoomBorderLoops(mapWidth, stat) {
    if (!stat.pixelIndices.length)
        return [];
    const occupied = new Set(stat.pixelIndices);
    const edges = [];
    let nextEdgeId = 0;
    ...
    for (const pixelIndex of stat.pixelIndices) {
        ...
        if (y === 0 || !occupied.has(topIndex))
            pushEdge(x, y, x + 1, y);
        if (x === mapWidth - 1 || !occupied.has(rightIndex))
            pushEdge(x + 1, y, x + 1, y + 1);
        if (!occupied.has(bottomIndex))
            pushEdge(x + 1, y + 1, x, y + 1);
        if (x === 0 || !occupied.has(leftIndex))
            pushEdge(x, y + 1, x, y);
    }
    ...
    for (const edge of edges) {
        ...
        while (true) {
            ...
            if (currentEdge.endKey === startKey)
                break;
...
function pointInPolygon(pointX, pointY, points) {
    ...
    for (let index = 0, previous = points.length - 1; index < points.length; previous = index++) {
        ...
        const intersects = (current.y > pointY) !== (prev.y > pointY) &&
...
function buildClipEraseMapGrid(mapData, source) {
    if (!source.eraseAreas.length)
        return undefined;
    ...
    for (const area of eraseAreas) {
        ...
        for (let y = startY; y <= endY; y++) {
            for (let x = startX; x <= endX; x++) {
                const index = y * width + x;
                if (!isRoomValue(clipEraseGrid[index] ?? 0))
                    continue;
...
function buildRoomTangentInfo(mapGrid, width, height) {
    const tangents = new Set();
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const current = mapGrid[y * width + x] ?? 0;
            if (!isRoomValue(current))
                continue;
            if (x + 1 < width) {
    ...
    return Array.from(tangents)
        ...
        return { roomID1, roomID2, tangent: 1 };
    })
        .sort((left, right) => left.roomID1 - right.roomID1 || left.roomID2 - right.roomID2);
}
function findAdjacentRegions(mapGrid, width, height, roomGridValues) {
    const roomIndexByGridValue = new Map();
    roomGridValues.forEach((roomGridValue, index) => roomIndexByGridValue.set(roomGridValue, index));
    const roomNeighborInfo = roomGridValues.map((roomGridValue) => [roomGridValue, new Set()]);
    for (let index = 0; index < mapGrid.length; index++) {
        ...
        for (let neighborY = y - 2; neighborY < y + 2; neighborY++) {
            for (let neighborX = x - 2; neighborX < x + 2; neighborX++) {
                ...
                if (nextRoomId === regionId || !roomIndexByGridValue.has(nextRoomId))
                    continue;
...
function reColoringMap(nextArea, color, colorPlan, matrix) {
    ...
    for (const availableColor of availableColors) {
        ...
        if (result.every((entry) => entry !== -1))
            return result;
    }
    return Array.from(colorPlan);
}
function buildRoomColorPlan(roomGridValues, matrix, roomCounts) {
    const roomColorMap = new Map();
    if (!roomGridValues.length)
        return roomColorMap;
    let maxRoomIndex = -1;
    let maxRoomCount = -1;
    ...
    for (const roomGridValue of roomGridValues) {
        ...
        if (currentColorId === 0)
            roomColorMap.set(roomGridValue, maxRoomColorId);
        else if (currentColorId === maxRoomColorId)
            roomColorMap.set(roomGridValue, 0);
    }
    return roomColorMap;
}
function parseSerializedRoomColorPlan(planStr, logicalRoomIdByGridValue) {
    if (!planStr?.trim())
        return null;
    try {
        ...
        if (!Array.isArray(plan) || !plan.length)
            return null;
        const gridValueByRoomId = new Map();
        ...
        for (const entry of plan) {
            ...
            if (!Number.isInteger(roomID) || !Number.isInteger(colorID))
                continue;
            if (colorID < 0 || colorID >= ROOM_COLOR_COUNT)
                continue;
...
function serializeRoomColorPlan(roomModels) {
    return JSON.stringify(roomModels.map((room) => ({
        roomID: room.roomID,
        colorID: room.colorID
    })));
}
function buildRoomModels(mapData, source, getDefaultRoomName, translateRoomName, mapGrid = mapData.mapGrid) {
    const roomMeta = new Map();
    for (const room of source.rooms)
        roomMeta.set(room.roomID, room);
    const logicalRoomIdByGridValue = new Map();
    for (const room of mapData.rooms ?? []) {
        if (room.gridValue != null)
            logicalRoomIdByGridValue.set(room.gridValue, room.roomId);
    }
    ...
    for (const [roomGridValue, stat] of stats)
        roomCounts.set(roomGridValue, stat.count);
    const roomColorPlan = parseSerializedRoomColorPlan(source.tempRoomColorPlanStr, logicalRoomIdByGridValue) ??
        buildRoomColorPlan(roomGridValues, findAdjacentRegions(mapGrid, mapData.header.sizeX, mapData.header.sizeY, roomGridValues), roomCounts);
    return roomGridValues.map((gridValue, index) => {
        ...
        const centerPoint = computeRoomLabelCenter(mapGrid, mapWidth, mapData.header.sizeY, gridValue, stat);
        const borderArr = buildRoomBorderLoops(mapWidth, stat);
        return {
            roomID: logicalRoomID,
            gridValue,
            roomName: (0, roomNameNormalizer_1.normalizeRoborockRoomDisplayName)(meta?.roomName, getDefaultRoomName, translateRoomName, meta?.roomType),
...
function buildObstaclePixels(source, entries, type) {
    return entries.map((entry) => ({
        type,
        point: devicePointToPixel(source, entry.point)
    }));
}
function buildSuspectedPixels(source) {
...
function buildPathPixels(source) {
    return source.pathPoints.map((point) => ({
        ...devicePointToPixel(source, point),
        type: point.type,
        update: point.update ?? q10PathTypeToHistoryUpdate(point.type)
    }));
}
function buildMaterialPathGroup(polygons) {
    if (!polygons?.length)
        return [];
    return polygons.map((polygon) => polygon.map((point) => mapArrPointToPixel(point)));
}
function buildSelfIdentifiedCarpets(data) {
    if (!data.carpetGrid?.length)
        return [];
...
function withVerification(mapData, q10CreatorData) {
    const nextMapData = { ...mapData, q10CreatorData };
    return {
        ...nextMapData,
        q10Verification: (0, Q10Verification_1.buildQ10Verification)(nextMapData)
    };
}
class Q10MapCreator {
    deps;
    ...
    translateRoomName(key, fallback) {
        return this.deps?.translationManager?.get(key, fallback) ?? fallback ?? key;
    }
    applyRuntimePose(mapData, source, creatorData, deviceStatus) {
        if (source.robotPosition ||
    ...
    create(mapData, deviceStatus) {
        ...
        const roomModels = buildRoomModels(mapData, source, () => this.getDefaultRoomName(), (key, fallback) => this.translateRoomName(key, fallback));
        const clipEraseMapGrid = buildClipEraseMapGrid(mapData, source);
        const clipEraseRoomModels = clipEraseMapGrid
            ? buildRoomModels(mapData, source, () => this.getDefaultRoomName(), (key, fallback) => this.translateRoomName(key, fallback), clipEraseMapGrid)
            : [];
        ...
        const { nextMapData, nextSource: runtimeSource, nextCreatorData } = this.applyRuntimePose(mapData, nextSource, q10CreatorData, deviceStatus);
...
exports.Q10MapCreator = Q10MapCreator;
//# sourceMappingURL=Q10MapCreator.js.map