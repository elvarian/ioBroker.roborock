...
exports.decompressQ10Lz4Block = decompressQ10Lz4Block;
exports.parseQ10PathOnlyToSourcePoints = parseQ10PathOnlyToSourcePoints;
exports.applyQ10PathOnlyToB01 = applyQ10PathOnlyToB01;
exports.mergeQ10PersistentState = mergeQ10PersistentState;
exports.mergeQ10RuntimeState = mergeQ10RuntimeState;
exports.applyQ10RuntimeStatePatch = applyQ10RuntimeStatePatch;
exports.parseQ10VirtualWallDpPayload = parseQ10VirtualWallDpPayload;
exports.parseQ10RestrictedZoneDpPayload = parseQ10RestrictedZoneDpPayload;
exports.parseQ10CarpetDpPayload = parseQ10CarpetDpPayload;
exports.parseQ10SuspectedPointsDpPayload = parseQ10SuspectedPointsDpPayload;
exports.isQ10YxMapPayload = isQ10YxMapPayload;
exports.parseQ10YxMapToB01 = parseQ10YxMapToB01;
...
function fixLikelyUtf8Mojibake(value) {
    if (!value || !/[ÃÂÐ]/u.test(value))
        return value;
    try {
...
function decodeRoomNameFromRaw(raw) {
...
function parseRoomData1(data) {
    ...
    pos += 1;
    if (data.length < 2 + regionNum * (26 + 20 + 1))
        return roomModels;
    for (let i = 0; i < regionNum; i++) {
        if (pos + 26 > data.length)
            break;
        ...
        if (pos + verticesBytes > data.length)
            break;
...
function factorPairForPixelCount(pixLen) {
    if (pixLen <= 0 || pixLen > MAX_MAP_PIXELS)
        return null;
...
function nearSquareDimensions(pixLen) {
...
function tryDecompressQ10Lz4Block(compressed, expectedSize, reasonOut) {
    ...
    try {
        while (inPos < input.length) {
            ...
            if (inPos + literalLength > input.length || outPos + literalLength > output.length) {
                if (reasonOut)
                    reasonOut.reason = "invalid literal block";
                return null;
            }
            ...
            if (inPos + 2 > input.length) {
                if (reasonOut)
                    reasonOut.reason = "invalid match offset";
                return null;
            }
            ...
            if (matchOffset <= 0 || matchOffset > outPos) {
                if (reasonOut)
                    reasonOut.reason = "invalid match distance";
                return null;
            }
            if (matchLength === 0x0f) {
                while (inPos < input.length) {
            ...
            if (outPos + matchLength > output.length) {
                if (reasonOut)
                    reasonOut.reason = "invalid match length";
                return null;
            }
        ...
        if (reasonOut)
            reasonOut.reason = e?.message || "decodeBlock threw";
        return null;
    }
}
function decompressQ10Lz4Block(compressed, expectedSize) {
...
function parseQ10HeaderAt(buf, offset, heightEndian) {
    if (buf.length < offset + Q10_HEADER_LEN)
        throw new Error("YxMap buffer too short");
...
function unwrapQ10OriginalMapPayload(buf) {
    if (!buf || buf.length < Q10_HEADER_LEN)
        return null;
    if (buf.length >= 1 + Q10_HEADER_LEN &&
...
function isStrictOriginalHeader(hdr, totalLen) {
    if (!hdr || hdr.version < 0 || hdr.version > 3)
        return false;
    if (hdr.mapWidth <= 0 || hdr.mapWidth > 4096)
        return false;
    if (hdr.mapHeight <= 0 || hdr.mapHeight > 4096)
        return false;
    if (hdr.mapWidth * hdr.mapHeight > MAX_MAP_PIXELS)
        return false;
    if (hdr.pixLen <= 0 || hdr.pixLen > MAX_MAP_PIXELS)
        return false;
    if (hdr.pixLzLen < 0)
        return false;
    if (hdr.mapResolution <= 0 || hdr.mapResolution > 1)
        return false;
    const pixelStart = hdr.offset + Q10_HEADER_LEN;
    if (pixelStart >= totalLen)
        return false;
    if (hdr.pixLzLen > 0) {
...
function isPlausibleHeader(hdr, totalLen) {
    if (!hdr || hdr.version < 0 || hdr.version > 3)
        return false;
    ...
    if (hdr.mapWidth < minDim || hdr.mapWidth > 4096)
        return false;
    if (hdr.mapHeight < minDim || hdr.mapHeight > 4096)
        return false;
    if (hdr.pixLen <= 0 || hdr.pixLen > MAX_MAP_PIXELS)
        return false;
...
function buildGrid(hdr, src) {
    ...
    if (version === 0) {
        ...
        for (let i = 0; i < pixByteCount && i < src.length; i++) {
            ...
            const toVal = (p) => (p === 0 ? 1 : p === 1 ? 128 : 127);
            if (i * 4 < size)
                mapGrid[i * 4] = toVal(p0);
            if (i * 4 + 1 < size)
                mapGrid[i * 4 + 1] = toVal(p1);
            if (i * 4 + 2 < size)
                mapGrid[i * 4 + 2] = toVal(p2);
            if (i * 4 + 3 < size)
                mapGrid[i * 4 + 3] = toVal(p3);
        }
    }
    else if (version === 2) {
...
function pixelDataLengthForVersion(version, mapWidth, mapHeight) {
    if (version === 0) {
...
function tryDecodeCandidate(buf, hdr, compatFallbacks = false) {
    const pixelStart = hdr.offset + Q10_HEADER_LEN;
    ...
    if (mapWidth === 0 || mapHeight === 0 || pixLen === 0)
        return null;
    if (mapWidth * mapHeight > MAX_MAP_PIXELS)
        return null;
    ...
    if (pixLzLen !== 0 && actualCompressedLen > 0) {
        ...
        if (!decompressed || decompressed.length !== pixLen)
            return null;
        src = decompressed;
    }
    else {
        if (pixelStart + pixLen > buf.length)
            return null;
        src = buf.subarray(pixelStart, pixelStart + pixLen);
    }
    const size = mapWidth * mapHeight;
    let mapGrid = buildGrid({ ...hdr, mapWidth, mapHeight, pixLen, pixLzLen }, src);
    if (!mapGrid || mapGrid.length !== size)
        return null;
    ...
    if (compatFallbacks && src.length > 0 && src.length < 0.1 * size) {
        ...
        let pair = null;
        if (n % 124 === 0 && n / 124 <= 2000)
            pair = { width: 124, height: n / 124 };
        else if (n % 256 === 0 && n / 256 <= 2000)
            pair = { width: 256, height: n / 256 };
        if (!pair)
            pair = factorPairForPixelCount(n);
        const ratio = pair ? Math.max(pair.width / pair.height, pair.height / pair.width) : Infinity;
        if (pair && ratio <= 15 && pair.width * pair.height === n) {
            const newGrid = buildGrid({ ...hdr, mapWidth: pair.width, mapHeight: pair.height, pixLen: n, pixLzLen: hdr.pixLzLen }, src);
    ...
    if (hdr.version !== 0) {
        ...
        if (pixDataLen > 0 && pixDataLen < src.length) {
            ...
            if (roomModels.length) {
                q10SourceData.rooms = roomModels.map((rm) => ({
                    roomID: rm.roomID,
    ...
    if ((hdr.chargeX !== 0 || hdr.chargeY !== 0) &&
        hdr.chargeX !== 0xffff &&
        hdr.chargeY !== 0xffff) {
    ...
    if (payloadEnd < buf.length) {
        try {
            ...
            if (trailing.obstaclePoints?.length) {
                q10SourceData.obstacles = trailing.obstaclePoints.map((point) => ({ point, type: "obstacle" }));
            ...
            if (trailing.skipCleanPoints?.length) {
                q10SourceData.skipPoints = trailing.skipCleanPoints.map((point) => ({ point, type: "skip" }));
...
function findInnerMapCandidate(buf) {
    const endianModes = ["le", "be"];
    let best = null;
...
function q10DevicePoint(dx, dy) {
    return {
        x: dx,
        y: dy
    };
}
function q10PathTypeToHistoryUpdate(type) {
    if (type === 0)
        return 6;
    if (type === 1)
        return 4;
...
function byte2ToShortBE(high, low) {
    const bytes = Buffer.from([high & 0xff, low & 0xff]);
    return bytes.readInt16BE(0);
}
function yxPathRleDecompress(compressed, compressedLen, outLen) {
    ...
    while (inPos < inputLen) {
        ...
        if (canFast && offset === 1) {
            ...
            while (srcPos < srcEnd)
                output[outPos++] = output[srcPos++] | 0;
        }
    }
    return output;
}
function parseQ10PathBlock(buf, offset) {
    try {
        const payloadLen = getQ10PathPayloadLengthAt(buf, offset);
        if (payloadLen === 0 || offset + 13 > buf.length)
            return null;
        let pos = offset;
...
function q10DevicePointToWorld(mapXMin, mapYMin, point) {
    return {
        x: mapXMin + point.x,
        y: mapYMin - point.y
    };
}
function q10DeviceAreaToWorld(mapXMin, mapYMin, area) {
    return {
        type: area.areaType,
...
function q10DeviceCarpetToWorld(mapXMin, mapYMin, area) {
    return {
        id: area.id,
        points: area.points.map((point) => q10DevicePointToWorld(mapXMin, mapYMin, point))
    };
}
function cloneDevicePoint(point) {
    return {
        x: point.x,
        y: point.y
    };
...
function cloneSuspectedPoint(point) {
    return {
        type: point.type,
        point: cloneDevicePoint(point.point)
    };
}
function cloneSourceArea(area) {
    return {
        id: area.id,
        type: area.type,
...
function buildQ10PathState(mapData, pathPoints) {
...
function rebuildQ10OverlayFields(mapData, source) {
...
function parseQ10PathOnlyToSourcePoints(buf) {
    ...
    const stripped = getQ10PathPayloadLengthAt(buf, 0) > 0
        ? buf
...
function applyQ10PathOnlyToB01(previous, pathPoints) {
    if (!previous.q10SourceData || !pathPoints.length)
        return previous;
    const nextSource = {
...
function mergeQ10PersistentState(current, previous) {
    if (!current?.q10SourceData || !previous?.q10SourceData)
        return current;
    const currentSource = current.q10SourceData;
    const previousSource = previous.q10SourceData;
    if (!currentSource.mapId)
        return current;
    if (previousSource.mapId !== 0 && currentSource.mapId !== previousSource.mapId)
        return current;
    let changed = false;
    const nextSource = {
        ...currentSource
...
function mergeQ10RuntimeState(current, previous) {
    if (!current?.q10SourceData || !previous?.q10SourceData)
        return current;
    const currentSource = current.q10SourceData;
    const previousSource = previous.q10SourceData;
...
function applyQ10RuntimeStatePatch(current, patch) {
...
function parseQ10VirtualWallDpPayload(buf) {
    if (!buf || buf.length < 1)
        return [];
    const count = buf[0] ?? 0;
    if (buf.length < 1 + count * 8)
        return [];
    const walls = [];
...
function parseQ10RestrictedZoneDpPayload(buf) {
    const result = {
        forbidAreas: [],
        mopAreas: [],
        thresholdAreas: []
    ...
    for (let index = 0; index < count; index++) {
        ...
        if (areaType === 2)
            result.mopAreas.push(area);
        else if (areaType === 3)
            result.thresholdAreas.push(area);
        else
            result.forbidAreas.push(area);
    }
    return result;
}
function parseQ10CarpetDpPayload(data) {
    ...
    for (const entry of data) {
        if (!entry || typeof entry !== "object" || Array.isArray(entry))
            continue;
        const item = entry;
        if (!Array.isArray(item.vertexs) || item.vertexs.length < 4)
            continue;
        const points = [];
...
function parseQ10SuspectedPointsDpPayload(data, type) {
    if (!Array.isArray(data))
        return [];
    const points = [];
    for (const entry of data) {
...
function parseQ10TrailingBlocks(buf, start, mapXMin, mapYMin) {
    void mapXMin;
    void mapYMin;
    const out = { nextOffset: start };
    ...
    if (erasesCount > 0) {
        ...
        pos += 1;
        if (vertsPerPoly < 2 || vertsPerPoly > 32)
            return out;
 ...
 */
function getQ10PathPayloadLengthAt(buf, offset) {
    if (!buf || offset + 13 > buf.length)
        return 0;
    const version = buf[offset];
    ...
    if (version < 0 || version > 3)
        return 0;
    if (pathMode < 0 || pathMode > 4)
        return 0;
    if (pointCount < 2 || pointCount > MAX_Q10_PATH_POINTS)
        return 0;
 ...
 */
function parseQ10EditTailSections(buf, start, mapXMin, mapYMin) {
    void mapXMin;
    void mapYMin;
    const out = {
        pathPoints: [],
    ...
    if (pos + virtualWallLen > buf.length)
        return out;
    ...
    for (let i = 0; i < forbidCount; i++) {
        ...
        const areaTypeName = areaType === 2 ? "mop" :
            areaType === 3 ? "threshold" :
                "forbid";
        const area = { type: areaTypeName, areaType, points };
        if (areaType === 2)
            out.mopZones.push(area);
        else if (areaType === 3)
            out.thresholdZones.push(area);
        else
            out.forbidAreas.push(area);
    }
    pos += forbidLen;
    if (pos + 1 > buf.length)
        return out;
    ...
    if (pos + carpetAreaLen > buf.length)
        return out;
...
function isQ10YxMapPayload(buf) {
    return buf != null && buf.length >= Q10_HEADER_LEN && buf[0] >= 0 && buf[0] <= 3;
}
function scoreDecodedCandidate(mapData) {
    if (!mapData?.header || !mapData.mapGrid?.length)
        return Number.NEGATIVE_INFINITY;
    const { sizeX, sizeY, minX, minY, maxX, maxY, resolution } = mapData.header;
    let score = 0;
    if (sizeX > 0 && sizeY > 0 && mapData.mapGrid.length === sizeX * sizeY)
        score += 100;
    if (sizeX >= 16 && sizeX <= 2000)
        score += 20;
    if (sizeY >= 16 && sizeY <= 2000)
        score += 20;
    if (resolution > 0 && resolution <= 1)
        score += 80;
    else if (resolution > 0 && resolution <= 5)
        score += 20;
    else
        score -= 120;
    ...
    if (Number.isFinite(spanX) && Number.isFinite(spanY)) {
        ...
        if (Math.abs(spanX - expectedSpanX) <= Math.max(resolution, 0.25))
            score += 15;
        if (Math.abs(spanY - expectedSpanY) <= Math.max(resolution, 0.25))
            score += 15;
    }
    // Real Q10 maps live in a small map-local coordinate system.
    const extentAbsMax = Math.max(Math.abs(minX), Math.abs(minY), Math.abs(maxX), Math.abs(maxY));
    if (extentAbsMax <= 200)
        score += 80;
    else if (extentAbsMax <= 2000)
        score += 20;
    else
        score -= 160;
    if (mapData.chargerPos) {
        const margin = Math.max(resolution * 2, 0.2);
        const inBounds = mapData.chargerPos.x >= minX - margin &&
            mapData.chargerPos.x <= maxX + margin &&
            mapData.chargerPos.y >= minY - margin &&
            mapData.chargerPos.y <= maxY + margin;
        score += inBounds ? 40 : -40;
        if (Math.abs(mapData.chargerPos.phi) <= 360)
            score += 10;
        else
            score -= 20;
    }
...
function tryDecodeHeaderCandidate(buf, offset, heightEndian) {
    try {
        const hdr = parseQ10HeaderAt(buf, offset, heightEndian);
        if (!isPlausibleHeader(hdr, buf.length))
            return null;
        if (hdr.version !== 0 && hdr.version !== 1 && hdr.version !== 2 && hdr.version !== 3)
            return null;
...
function parseStrictOriginalQ10Map(buf) {
    const payload = unwrapQ10OriginalMapPayload(buf);
    if (!payload || payload.length < Q10_HEADER_LEN)
        return null;
    try {
        const hdr = parseQ10HeaderAt(payload, 0, "be");
        if (!isStrictOriginalHeader(hdr, payload.length))
            return null;
 ...
 */
function parseQ10YxMapToB01(buf) {
    if (!buf || buf.length < Q10_HEADER_LEN)
        return null;
    const strict = parseStrictOriginalQ10Map(buf);
    if (strict)
        return strict;
    if (!Q10_COMPAT_FALLBACKS_ENABLED)
        return null;
    ...
    for (const candidate of preferredCandidates) {
        if (candidate.offset < 0 || candidate.offset + Q10_HEADER_LEN > buf.length)
            continue;
...
//# sourceMappingURL=Q10YxMapParser.js.map