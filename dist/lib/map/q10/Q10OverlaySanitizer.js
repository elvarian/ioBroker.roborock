...
exports.sanitizeQ10SourceOverlayAreas = sanitizeQ10SourceOverlayAreas;
function isFiniteNumber(value) {
    return typeof value === "number" && Number.isFinite(value);
}
function hasStructurallyValidPoints(area) {
    if (!area?.points?.length)
        return false;
    return area.points.every((point) => isFiniteNumber(point?.x) && isFiniteNumber(point?.y));
}
function isStructurallyValidArea(area) {
    if (!hasStructurallyValidPoints(area))
        return false;
...
function filterOverlayAreas(areas) {
 ...
 */
function sanitizeQ10SourceOverlayAreas(header, source) {
    void header;
    ...
    const changed = virtualWalls.length !== source.virtualWalls.length ||
...
//# sourceMappingURL=Q10OverlaySanitizer.js.map