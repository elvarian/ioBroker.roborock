...
exports.cryptoEngine = {
    ...
    ensureRsaKeys() {
        ...
        const kp = node_forge_1.default.pki.rsa.generateKeyPair(1024);
        const padHex = (hex) => (hex.length % 2 === 1 ? "0" + hex : hex);
        rsaKeys = {
            public: {
    ...
    decryptRSA(ciphertext) {
        ...
        const privateKey = node_forge_1.default.pki.setRsaPrivateKey(new node_forge_1.default.jsbn.BigInteger(keys.private.n, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.e, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.d, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.p, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.q, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.dmp1, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.dmq1, 16), new node_forge_1.default.jsbn.BigInteger(keys.private.coeff, 16));
        let decryptedRawStr = null;
        try {
     ...
     */
    decryptAES_CBC(ciphertext, key, iv) {
     ...
     */
    decryptPhotoPayload(encryptedData) {
        for (let offset = 0; offset < 256; offset++) {
            if (offset + 128 > encryptedData.length)
                break;
            try {
                const block = encryptedData.subarray(offset, offset + 128);
                let decryptedKeyBlock = null;
                try {
