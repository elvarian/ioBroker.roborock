...
class socketHandler {
    ...
    async handleGetObstacleImage(msg) {
        ...
        if (obstacleId === undefined || obstacleId === null) {
            ...
            if (msg.callback)
                this.adapter.sendTo(msg.from, msg.command, { error: "Missing ID" }, msg.callback);
            return;
        }
            ...
            if (Array.isArray(segmentIds) && segmentIds.length > 0) {
                for (const id of segmentIds) {
                    ...
                    if (name && String(name).trim())
                        result[String(id)] = String(name).trim();
                }
            }
            else {
                const ns = this.adapter.namespace;
                const list = await this.adapter.getObjectListAsync(ns, {
                    type: "state",
                    startkey: `${ns}.${prefix}`,
                    endkey: `${ns}.${prefix}\u9999`
                });
                const rows = list?.rows ?? (Array.isArray(list) ? list : []);
                for (const row of rows) {
                    ...
                    if (name && String(name).trim())
                        result[segId] = String(name).trim();
                }
            }
            if (msg.callback)
                this.adapter.sendTo(msg.from, msg.command, result, msg.callback);
        }
        catch (error) {
     ...
     */
    async handleSimpleCommand(duid, command, id) {
        if (!duid)
     ...
     */
    async handleGotoTarget(message, id) {
     ...
     */
    async handleZonedClean(message, id) {
