...
exports.requestsHandler = exports.RoborockRequest = exports.RequestPriority = void 0;
const p_queue_1 = __importDefault(require("p-queue"));
const Q10CommandHandler_1 = require("./b01/q10/Q10CommandHandler");
...
function isRetryableError(error) {
    return (0, errorUtils_1.isConnectivityLikeError)(error);
}
var RequestPriority;
(function (RequestPriority) {
    RequestPriority[RequestPriority["LOW"] = -10] = "LOW";
    RequestPriority[RequestPriority["NORMAL"] = 0] = "NORMAL";
    RequestPriority[RequestPriority["HIGH"] = 10] = "HIGH";
...
class RequestManager {
    ...
    async add(id, taskFunction, priority = RequestPriority.NORMAL) {
...
class RoborockRequest {
    ...
    rejectPromise;
    promise;
    version; // Store protocol version for logging
    creationTime;
    startTime = 0;
    binaryType;
    sentConnectionType;
    ...
    manager;
    queueName;
    constructor(handler, duid, method, params, manager, queueName, version = "1.0", timeout = REQUEST_TIMEOUT) {
    ...
    resolve(result, version) {
        if (this.timeoutTimer)
            this.adapter.clearTimeout(this.timeoutTimer);
...
exports.RoborockRequest = RoborockRequest;
class requestsHandler {
    adapter;
    idCounter;
    photoIdCounter;
    lastB01Id; // For B01 timestamp-based IDs
    globalManager; // For commands (high concurrency)
    ...
    getQ10CommandHandler() {
        if (!this.q10CommandHandler) {
            this.q10CommandHandler = new Q10CommandHandler_1.Q10CommandHandler(this);
        }
    ...
    static getLogLevelForMethod(method) {
    ...
    _processResult(requestPromise, callback, identifier, duid) {
    ...
    async sendRequest(duid, method, params, options = {}) {
        ...
        const isB01MapMethod = method === "get_map_v1" ||
        ...
        const attempt = async (retryCount) => {
            ...
            try {
                ...
                if (retryCount < MAX_REQUEST_RETRIES && isRetryableError(error)) {
                    this.adapter.rLog("System", duid, "Warn", "Retry", undefined, `[sendRequest] ${method} failed (${error.message}). Retrying (${retryCount + 1}/${MAX_REQUEST_RETRIES + 1})...`, "warn");
                    await new Promise((resolve) => setTimeout(resolve, 1000));
                    return attempt(retryCount + 1);
                }
                throw error;
    ...
    async command(_handler, duid, method, params, id) {
        ...
        this._processResult(requestPromise, async (res) => {
    ...
    resolvePendingRequest(messageID, result, protocol, duid, connectionType = "Unknown", version) {
        const req = this.adapter.pendingRequests.get(messageID);
        if (req) {
            const reqDuid = req.duid || (duid || "unknown");
            ...
            if (protocol) {
                ...
                const reqVersion = req.version || version || "1.0";
                ...
                this.adapter.rLog(connectionType, reqDuid, "<-", reqVersion, String(protocol), `${method}: ${extraInfo} | ${durationStr}`, logLevel, logMsgId);
            }
            ...
            if (!isPollMethod) {
                ...
                if (handler) {
                    handler.updateStatus().catch((e) => {
                        this.adapter.rLog("System", reqDuid, "Error", "Command", undefined, `Status update after ${method}: ${e.message}`, "error");
                    });
                }
            }
    ...
    rejectPendingTcpRequests(duid, reason) {
        let rejected = 0;
        for (const req of Array.from(this.adapter.pendingRequests.values())) {
            if (!(req instanceof RoborockRequest))
                continue;
            if (req.duid !== duid || req.sentConnectionType !== "TCP")
                continue;
    ...
    getPendingMapRequest(duid) {
        let found;
        for (const req of this.adapter.pendingRequests.values()) {
            const r = req;
            if ((r?.method === "get_map_v1" || r?.method === "get_clean_record_map") && r.duid === duid) {
                if (found)
                    return undefined; // more than one, don't guess
                found = r;
            }
        }
        return found;
     ...
     */
    getPendingBinaryRequest(payloadBuf, duid) {
        if (payloadBuf.length < 4)
            return undefined;
        try {
