 ...
 */
exports.BaseStatusSchema = zod_1.z.looseObject({
...
class BaseDeviceFeatures {
    createdStates = new Set(); // Track created states to avoid redundant ensureState calls
    ...
    constructor(dependencies, duid, robotModel, config) {
        ...
        this.config = config;
        this.stateWriter = new deviceStateWriter_1.DeviceStateWriter(dependencies, duid);
    ...
    async applyFeature(feature) {
        ...
        if (registry && registry.has(feature)) {
            ...
            try {
                const applyMethod = this[methodName];
                if (typeof applyMethod !== "function")
                    throw new Error(`Feature ${String(feature)}: missing method ${methodName}`);
     ...
     */
    addCommand(name, spec, group = "commands") {
    ...
    async processResultKey(folder, key, val) {
        ...
        if (!common) {
            common = { name: key, type: typeof val, read: true, write: false };
        }
        ...
        if ((key === "last_clean_t" || key === "begin" || key === "end") && typeof val === "number") {
        ...
        await this.deps.adapter.setStateChanged(fullPath, { val: val, ack: true });
    }
    // --- Helper Methods ---
    ...
    async getPhoto(imgId, type) {
        if (!this.hasFeature(features_enum_1.Feature.GetPhoto)) {
            throw new Error("getPhoto feature not enabled for this device");
        }
        try {
            const res = (await this.deps.adapter.requestsHandler.sendRequest(this.duid, "get_photo", {
                data_filter: {
                    img_id: imgId,
                    type: type,
                },
            ...
            const responseData = res.buffer ? res : res.data || res;
            return responseData;
        }
        catch (e) {
