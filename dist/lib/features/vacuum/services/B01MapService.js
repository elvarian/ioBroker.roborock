...
class B01MapService {
    ...
    onRoomsDetected;
    stateWriter;
    lastQ10LiveMapBlob = null;
    lastQ10PathBlob = null;
    static isLiveMapPayload(data) {
        return (0, B01MapPayloadClassifier_1.classifyB01MapPayload)(data).kind === "live";
    }
    constructor(deps, duid, onRoomsDetected) {
        this.deps = deps;
        this.duid = duid;
        this.onRoomsDetected = onRoomsDetected;
            ...
            this.deps.adapter.pendingRequests.set(dummyId, {
                ...
                resolve: (data) => {
                    ...
                    if (Buffer.isBuffer(data)) {
                        ...
                        if (!B01MapService.isLiveMapPayload(data))
                            return;
    ...
    async applyLiveMapPayload(data) {
        await this.processUpdateMapResponse(data);
    }
    async processUpdateMapResponse(data) {
        const payloadClassification = (0, B01MapPayloadClassifier_1.classifyB01MapPayload)(data);
        if (payloadClassification.kind !== "live")
            return;
        const isQ10PathOnlyPayload = payloadClassification.variant === "q10" &&
        ...
        const mapRes = await this.deps.adapter.mapManager.processMap(data, "B01", this.deps.adapter.http_api.getRobotModel(this.duid) || "B01", serial, null, this.duid, "B01");
        if (mapRes) {
    ...
    async updateMultiMapsList() {
        try {
            ...
            const mapList = (result && typeof result === "object" && result.map_list) || (result && typeof result === "object" && result.data?.map_list);
            ...
            this.deps.adapter.rLog("System", this.duid, "Info", "B01", undefined, `B01 floors updated: ${mapList.length} maps (${mapList.map((m) => m.name || m.id).join(", ")})`, "info");
        }
        catch (e) {
     ...
     */
    async getCleaningRecordMap(startTime, recordDetails, recordIndex) {
        try {
    ...
    async processHistoryMapResponse(data, resolve) {
        try {
            ...
            const mapRes = await this.deps.adapter.mapManager.processMap(data, "B01", this.deps.adapter.http_api.getRobotModel(this.duid) || "B01", serial, null, this.duid, "B01History");
            resolve(mapRes);
        }
        catch (e) {
    ...
    async updateRoomMapping(mappedRooms, options = {}) {
        if (!mappedRooms)
            return;
        ...
        for (const room of rooms) {
            ...
            const rawRoomName = typeof room.name === "string" && room.name.trim()
                ...
                : typeof room.roomName === "string"
                    ? room.roomName
                    : "";
            const roomName = (0, roomNameNormalizer_1.normalizeRoborockRoomDisplayName)(rawRoomName, () => this.getDefaultRoomName(), (key, fallback) => this.translateRoomName(key, fallback), typeof room?.roomTypeId === "number" ? room.roomTypeId : undefined);
...
exports.B01MapService = B01MapService;
//# sourceMappingURL=B01MapService.js.map