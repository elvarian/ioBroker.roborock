...
exports.Q10ShadowDataService = void 0;
const deviceStateWriter_1 = require("../../../deviceStateWriter");
const Q10YxMapParser_1 = require("../../../../map/q10/Q10YxMapParser");
const roomNameNormalizer_1 = require("../../../../roomNameNormalizer");
const vacuumConstants_1 = require("../../vacuumConstants");
class Q10ShadowDataService {
    deps;
    duid;
    host;
    q10SkipNextTimerRefreshFromDp93 = false;
    q10AwaitingTimerListResult = false;
    stateWriter;
    constructor(deps, duid, host) {
        this.deps = deps;
        this.duid = duid;
        this.host = host;
        this.stateWriter = new deviceStateWriter_1.DeviceStateWriter(deps, duid);
    }
    skipNextTimerRefreshFromDp93() {
        this.q10SkipNextTimerRefreshFromDp93 = true;
    }
    markTimerListRequestPending() {
        this.q10AwaitingTimerListResult = true;
    }
    normalizeQ10BooleanNumberFlags(statusData) {
        for (const key of ["map_save_switch", "recent_clean_record", "valley_point_charging"]) {
            if (typeof statusData[key] === "boolean") {
                statusData[key] = statusData[key] ? 1 : 0;
            }
        }
    }
    decodeQ10ShadowBytes(value) {
    ...
    q10WeekDataToWeekArray(weekData) {
        const weeks = [];
        if (((weekData >> 6) & 1) === 1) {
            weeks.push(0);
        }
    ...
    async resolveQ10FloorName(mapId) {
    ...
    async applyQ10CurrentCleanRoomIds(dp91) {
        let roomIds = [];
    ...
    async applyQ10ShadowConsumables(topLevelDps, commonDps) {
        ...
        if (!availableShadowConsumables.length)
            return;
    ...
    getLocalizedConsumableName(deviceName) {
    ...
    getConsumableLifeSpanHours(deviceName) {
        switch (deviceName) {
    ...
    normalizeQ10ShadowConsumableHours(deviceName, value) {
    ...
    async applyQ10StatusFromDpResult(dpResult) {
        const resultObj = { ...dpResult };
        if (resultObj.state !== undefined)
            resultObj.status = resultObj.state;
        if (resultObj.fan_power !== undefined)
            resultObj.wind = resultObj.fan_power;
        if (resultObj.water_box_mode !== undefined)
            resultObj.water = resultObj.water_box_mode;
        try {
    ...
    async applyQ10MapInfoFromDpResult(resultItem) {
        ...
        if (!Array.isArray(mapInfoList) || mapInfoList.length === 0)
            return;
        try {
            ...
            for (const map of mapInfoList) {
                ...
                for (const room of rooms) {
                    ...
                    const rawRoomName = typeof r.iot_name === "string" && r.iot_name.trim()
                        ? r.iot_name
                        : typeof r.name === "string"
                            ? r.name
                            : "";
                    const roomName = (0, roomNameNormalizer_1.normalizeRoborockRoomDisplayName)(rawRoomName, () => this.getDefaultRoomName());
    ...
    async applyQ10NotDisturbData(dpValue) {
        ...
        try {
            ...
            await this.host.processStatusProperty("not_disturb_time", `${this.formatQ10Time(bytes[1] ?? 0)}:${this.formatQ10Time(bytes[2] ?? 0)}-${this.formatQ10Time(bytes[3] ?? 0)}:${this.formatQ10Time(bytes[4] ?? 0)}`);
        }
        catch (e) {
    ...
    async applyQ10ValleyPointChargingData(dpValue) {
        ...
        try {
            ...
            await this.host.processStatusProperty("valley_point_charging_time", `${this.formatQ10Time(bytes[1] ?? 0)}:${this.formatQ10Time(bytes[2] ?? 0)}-${this.formatQ10Time(bytes[3] ?? 0)}:${this.formatQ10Time(bytes[4] ?? 0)}`);
        }
        catch (e) {
    ...
    async applyQ10ShadowDpPayload(dps) {
        ...
        if (common && typeof common === "object" && !Array.isArray(common)) {
            ...
            if (commonDps["79"] && typeof commonDps["79"] === "object" && !Array.isArray(commonDps["79"])) {
                const timeZone = commonDps["79"];
                if (timeZone.timeZoneSec !== undefined)
                    statusData.time_zone = timeZone.timeZoneSec;
                if (timeZone.timeZoneCity !== undefined)
                    statusData.time_zone_info = timeZone.timeZoneCity;
            }
            if (commonDps["92"] && typeof commonDps["92"] === "object" && !Array.isArray(commonDps["92"])) {
                const disturb = commonDps["92"];
                if (disturb.disturb_light !== undefined)
                    statusData.disturb_light = disturb.disturb_light;
                if (disturb.disturb_voice !== undefined)
                    statusData.disturb_voice = disturb.disturb_voice;
                if (disturb.disturb_resume_clean !== undefined)
                    statusData.disturb_resume_clean = disturb.disturb_resume_clean;
                if (disturb.disturb_dust_enable !== undefined)
                    statusData.disturb_dust_enable = disturb.disturb_dust_enable;
            }
            const multiMapList = commonDps["61"];
            if (multiMapList && typeof multiMapList === "object" && !Array.isArray(multiMapList)) {
                await this.applyQ10MultiMapListFromDp61(multiMapList);
            }
            const cleanRecords = commonDps["52"];
            if (cleanRecords && typeof cleanRecords === "object" && !Array.isArray(cleanRecords)) {
                await this.host.applyCleanRecordList(cleanRecords);
            }
            ...
            if (commonDps["55"] !== undefined) {
                ...
                if (bytes) {
                    const restricted = (0, Q10YxMapParser_1.parseQ10RestrictedZoneDpPayload)(Buffer.from(bytes));
            ...
            if (commonDps["57"] !== undefined) {
                ...
                if (bytes) {
                    liveMapPatch.virtualWalls = (0, Q10YxMapParser_1.parseQ10VirtualWallDpPayload)(Buffer.from(bytes));
                }
            }
            if (commonDps["65"] && typeof commonDps["65"] === "object" && !Array.isArray(commonDps["65"])) {
                const dp65 = commonDps["65"];
                if (Array.isArray(dp65.data)) {
                    liveMapPatch.carpetAreas = (0, Q10YxMapParser_1.parseQ10CarpetDpPayload)(dp65.data);
                }
            }
            if (commonDps["98"] !== undefined) {
                liveMapPatch.suspectedPoints = [
                    ...(liveMapPatch.suspectedPoints ?? []),
                    ...(0, Q10YxMapParser_1.parseQ10SuspectedPointsDpPayload)(commonDps["98"], "easycard")
                ];
            }
            if (commonDps["100"] !== undefined) {
                liveMapPatch.suspectedPoints = [
                    ...(liveMapPatch.suspectedPoints ?? []),
                    ...(0, Q10YxMapParser_1.parseQ10SuspectedPointsDpPayload)(commonDps["100"], "threshold")
                ];
            }
            if (commonDps["103"] !== undefined) {
                liveMapPatch.suspectedPoints = [
                    ...(liveMapPatch.suspectedPoints ?? []),
                    ...(0, Q10YxMapParser_1.parseQ10SuspectedPointsDpPayload)(commonDps["103"], "cliff")
                ];
            }
            ...
            if (candidateNet81 && typeof candidateNet81 === "object" && !Array.isArray(candidateNet81)) {
                net81 = candidateNet81;
            }
            await this.applyQ10ShadowConsumables(dps, commonDps);
        }
        if (Object.keys(statusData).length > 0) {
            if (statusData.fan_power !== undefined)
                statusData.wind = statusData.fan_power;
            if (statusData.water_box_mode !== undefined)
                statusData.water = statusData.water_box_mode;
        ...
        if ((liveMapPatch.virtualWalls !== undefined ||
            ...
            typeof this.deps.adapter.mapManager?.applyQ10LiveStatePatch === "function") {
            await this.deps.adapter.mapManager.applyQ10LiveStatePatch(this.duid, liveMapPatch);
        }
    }
}
exports.Q10ShadowDataService = Q10ShadowDataService;
//# sourceMappingURL=Q10ShadowDataService.js.map