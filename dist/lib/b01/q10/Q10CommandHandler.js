...
exports.Q10CommandHandler = void 0;
const Q10_NATIVE_DP_COMMANDS = {
    app_start: { "201": { cmd: 1 } },
    app_pause: { "204": 0 },
    app_stop: { "206": 0 },
    app_charge: { "202": 5 }
...
function encodeSignedInt16Be(value) {
    const buffer = Buffer.allocUnsafe(2);
    buffer.writeInt16BE(Math.trunc(value), 0);
    return [buffer[0], buffer[1]];
}
function encodeQ10ZoneCleanParameters({ repeatCount, areas }) {
    const bytes = [1, repeatCount, areas.length];
...
class Q10CommandHandler {
    requestHandler;
    constructor(requestHandler) {
        this.requestHandler = requestHandler;
    }
    async resolveCommandParams(handler, method, params) {
    ...
    async publishNativeQ10DpCommand(duid, method, dps) {
        this.requestHandler.adapter.rLog("System", duid, "Debug", "B01", undefined, `Q10 command ${method}: using source-verified Q10 DP path`, "debug");
    ...
    async handleCommand(handler, duid, method, params) {
...
exports.Q10CommandHandler = Q10CommandHandler;
//# sourceMappingURL=Q10CommandHandler.js.map