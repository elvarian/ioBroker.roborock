...
const decryptors = {
    ...
    "\x81S\x19": (payload, key, random) => cryptoEngine_1.cryptoEngine.decryptB01(payload, key, random),
};
const encryptors = {
    ...
    "\x81S\x19": (payload, key, random) => cryptoEngine_1.cryptoEngine.encryptB01(payload, key, random),
};
class messageParser {
    adapter;
    transportSequences = new Map();
    constructor(adapter) {
        this.adapter = adapter;
    }
    resetTransportSequence(duid, nextSequenceId = 1) {
    ...
    decodeMsg(message, duid) {
        ...
        while (offset + 3 <= message.length) {
            ...
            if (version === "B01" && (data.protocol === 300 || data.protocol === 301 || data.protocol === 302)) {
                data.rawPayload = Buffer.from(data.payload);
                data.rawFrame = Buffer.from(msgBuffer);
            }
            // Decrypt
            try {
                if (version === "L01") {
     ...
     */
    async buildRoborockMessage(duid, protocol, timestamp, payload, version, sequenceId) {
