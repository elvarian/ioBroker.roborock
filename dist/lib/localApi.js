...
function nextSocketRandom() {
    return Math.floor(Math.random() * 1_000_000 + 1_000) >>> 0;
}
class EnhancedSocket extends node_net_1.Socket {
    connected;
    chunkBuffer;
    receivedBytes;
    sentBytes;
...
class local_api {
    ...
    sessionAckWaiters = new Map();
    discoveryServer = null;
    discoveryTimer = null;
    gracePeriodTimer = null;
    discoveryRestartTimer = null;
    discoveryWindowPromise = null;
    resolveDiscoveryWindow = null;
    discoveryStopping = false;
    endpointRefreshPromises = new Map();
    endpointRefreshLastStartedAt = new Map();
    tcpKeepaliveInterval = undefined;
    static TCP_KEEPALIVE_MS = 10_000;
    static TCP_KEEPALIVE_GRACE_MS = 1_000;
    static TCP_KEEPALIVE_CHECK_MS = 1_000;
    static UDP_DISCOVERY_RESTART_MS = 30_000;
    static ENDPOINT_REFRESH_MIN_INTERVAL_MS = 60_000;
    static STALE_ENDPOINT_CONNECT_MIN_INTERVAL_MS = 60_000;
    constructor(adapter) {
     ...
     */
    async initiateClient(duid, suppressLog = false, timeoutMs = 5000) {
    ...
    async _performConnection(duid, timeoutMs) {
        ...
        client.on("data", async (message) => {
            try {
                ...
                client.chunkBuffer = client.chunkBuffer.length === 0
                    ? message
                    : Buffer.concat([client.chunkBuffer, message]);
                let offset = 0;
    ...
    checkTcpActivity(duid) {
        const client = this.deviceSockets[duid];
        if (!client?.connected || !this.isConnected(duid))
            return;
        const version = this.getLocalProtocolVersion(duid);
        if (version !== "1.0" && version !== "L01")
            return;
        ...
        if (inboundIdleMs < pingDueMs && outboundIdleMs < pingDueMs)
            return;
        this.sendPing(duid);
    }
    stopTcpKeepaliveInterval() {
        if (this.tcpKeepaliveInterval) {
            this.adapter.clearInterval(this.tcpKeepaliveInterval);
            this.tcpKeepaliveInterval = undefined;
        }
    }
    resetDeviceSocket(duid, reason, rejectPendingRequests = true) {
    ...
    markEndpointStale(duid, reason) {
        ...
        this.refreshEndpoint(duid, reason).catch((e) => {
    ...
    scheduleReconnect(duid, reason, silent = false) {
        ...
        this.adapter.setTimeout(() => {
            ...
            if (this.localDevices[duid]?.staleSince) {
                this.refreshEndpoint(duid, reason).catch((e) => this.adapter.rLog("TCP", duid, "Debug", undefined, undefined, `Endpoint refresh failed: ${this.adapter.errorMessage(e)}`, "debug"));
            }
    ...
    waitForSessionAck(duid, timeoutMs, version) {
    ...
    resolveSessionAck(duid) {
        const waiter = this.sessionAckWaiters.get(duid);
        if (waiter) {
            waiter.resolve();
        }
    }
    rejectSessionAck(duid, error) {
        const waiter = this.sessionAckWaiters.get(duid);
        if (waiter) {
    ...
    resolveLocalProtocol4Payload(duid, version, protocol, parsedPayload) {
        const dps = parsedPayload?.dps;
        let content = null;
        ...
        if (!content || typeof content !== "object" || typeof content.id === "undefined")
            return;
    ...
    extractIpFromNetworkInfo(result) {
        ...
        return (0, node_net_1.isIP)(ip) !== 0 ? ip : null;
    }
    connectLocalEndpointIfDue(duid, timeoutMs = 5000, minIntervalMs = local_api.STALE_ENDPOINT_CONNECT_MIN_INTERVAL_MS) {
        ...
        if (!dev)
            return;
        const now = Date.now();
        if (dev.lastConnectAttemptAt && now - dev.lastConnectAttemptAt < minIntervalMs)
            return;
        dev.lastConnectAttemptAt = now;
        this.initiateClient(duid, true, timeoutMs).catch((e) => {
    ...
    updateLocalEndpoint(duid, ip, version, source = "network_info") {
        if (!duid || typeof duid !== "string" || !ip || (0, node_net_1.isIP)(ip) === 0 || !version)
            return false;
        const now = Date.now();
    ...
    async refreshEndpoint(duid, reason = "endpoint refresh", force = false) {
    ...
    async refreshEndpointInternal(duid, reason) {
    ...
    async refreshStaleLocalEndpoints(reason = "scheduled endpoint refresh") {
        const duids = Object.keys(this.localDevices).filter((duid) => {
            const dev = this.localDevices[duid];
            return !!dev?.ip && !this.isConnected(duid);
        });
        await Promise.all(duids.map((duid) => this.refreshEndpoint(duid, reason).catch((e) => {
    ...
    async startUdpDiscovery(timeoutMs = 10_000) {
        ...
        if (timeoutMs <= 0)
            return;
        if (this.discoveryWindowPromise)
    ...
    scheduleUdpDiscoveryRestart(reason) {
        if (this.discoveryStopping || this.discoveryRestartTimer)
            return;
    ...
    handleUdpDiscoveryMessage(msg) {
        ...
        try {
            ...
            if (!parsedDecodedMessage || typeof parsedDecodedMessage !== "object")
                return;
            const duid = parsedDecodedMessage.duid;
            const ip = parsedDecodedMessage.ip;
            if (typeof duid !== "string" || typeof ip !== "string")
                return;
        ...
        return new Promise((resolve) => {
            ...
            const checkFinished = () => {
                const freshOwnedCount = this.getFreshDiscoveryDuids(startedAt, true)
                    ...
                    .length;
                if (freshOwnedCount < expectedOwnedDuids.size || this.gracePeriodTimer)
                    return;
                if (this.discoveryTimer) {
                    clearTimeout(this.discoveryTimer);
                    this.discoveryTimer = null;
     ...
     */
    async checkAndPromoteLocalConnection(duid, ip, timeoutMs = 5000, suppressLog = false) {
        if (this.isConnected(duid))
            return true; // Register temporarily to allow initiateClient to work
    ...
    sendPubAck(duid, messageId, version) {
    ...
    buildControlFrame(version, messageId, randomValue, timestamp, protocol, payload = Buffer.alloc(0)) {
