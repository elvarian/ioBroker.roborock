...
(0, vitest_1.describe)("Command Verification", () => {
    let mockAdapter;
    let mockRobot;
    let vacuumFeatures;
    let depsMock;
    ...
    (0, vitest_1.it)("should send app_start command when state is set", async () => {
        ...
        (0, vitest_1.expect)(commands).to.have.property("app_start");
        ...
        (0, vitest_1.expect)(lastMethod).to.equal("app_start");
        (0, vitest_1.expect)(mockRobot.state.in_cleaning).to.equal(1); // MockRobot should switch state
    });
    (0, vitest_1.it)("should send custom mode (fan power) correctly", async () => {
        let lastParams = [];
        const originalHandleRequest = mockRobot.handleRequest.bind(mockRobot);
        mockRobot.handleRequest = (method, params) => {
        ...
        (0, vitest_1.expect)(lastParams).to.deep.equal([105]);
        (0, vitest_1.expect)(mockRobot.state.fan_power).to.equal(105);
    });
    (0, vitest_1.it)("should reset consumables correctly", async () => {
        ...
        (0, vitest_1.expect)(mockRobot.consumables.main_brush_work_time).to.equal(0);
    });
    (0, vitest_1.it)("should handle complex carpet_mode JSON", async () => {
        let lastParams = [];
        const originalHandleRequest = mockRobot.handleRequest.bind(mockRobot);
        mockRobot.handleRequest = (method, params) => {
        ...
        (0, vitest_1.expect)(lastParams[0]).to.deep.equal(complexMode);
    });
});
