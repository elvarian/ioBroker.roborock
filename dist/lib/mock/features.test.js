...
class TestVacuumFeatures extends v1VacuumFeatures_1.V1VacuumFeatures {
    ...
    isResetableConsumable() {
        return false;
    }
    getCommonDeviceStates() {
        return {};
    }
    getCommonCleaningRecords() {
        return {};
    }
    getFirmwareFeatureName() {
        return "";
    }
    getCommonCleaningInfo() {
        return {};
    }
...
(0, vitest_1.describe)("Features - State Creation", () => {
    let mockAdapter;
    let mockRobot;
    ...
    (0, vitest_1.it)("should create dockingStationStatus states only when supported", async () => {
        const deps = {
            adapter: mockAdapter,
            log: {
                info: console.log,
        ...
        deps.adapter.translationManager = { get: (key, def) => def || key };
        ...
        (0, vitest_1.expect)(mockAdapter.objects).to.not.have.property(dssFolder);
        ...
        (0, vitest_1.expect)(mockAdapter.objects, "dockingStationStatus folder missing after feature applied").to.have.property(dssFolder);
        (0, vitest_1.expect)(mockAdapter.objects, "cleanFluidStatus state missing after feature applied").to.have.property(cleanFluid);
    });
    (0, vitest_1.it)("should dynamically create resetConsumables states based on data", async () => {
        ...
        (0, vitest_1.expect)(mockAdapter.objects).to.not.have.property(resetMainBrush);
        ...
        (0, vitest_1.expect)(mockAdapter.objects).to.have.property(resetMainBrushPath);
        (0, vitest_1.expect)(mockAdapter.objects).to.have.property(resetSideBrush);
        (0, vitest_1.expect)(mockAdapter.objects).to.have.property(resetFilter);
        // Should NOT create reset button for unknown consumable (only work_time/dirty_time suffixes)
        (0, vitest_1.expect)(mockAdapter.objects).to.not.have.property(resetUnknown);
        // Verify object properties for a created button
        const btnObj = mockAdapter.objects[resetMainBrushPath];
        (0, vitest_1.expect)(btnObj.common.role).to.equal("button");
        (0, vitest_1.expect)(btnObj.common.type).to.equal("boolean");
        (0, vitest_1.expect)(btnObj.common.write).to.equal(true);
    });
});
