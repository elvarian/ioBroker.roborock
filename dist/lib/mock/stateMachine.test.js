...
(0, vitest_1.describe)("State Machine Deep Dive", () => {
    let mockAdapter;
    let mockRobot;
    let vacuumFeatures;
    let depsMock;
    ...
    (0, vitest_1.it)("should process cleaning -> error -> resolved -> charging transition", async () => {
        ...
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.state`]).to.equal(5);
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.in_cleaning`]).to.equal(1);
        // 2. Error State: Device Stuck
        mockRobot.state.state = 12; // Error
        mockRobot.state.error_code = 8; // Stuck
        mockRobot.state.in_cleaning = 0;
        await vacuumFeatures.updateStatus();
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.state`]).to.equal(12);
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.error_code`]).to.equal(8);
        // 3. Resolved: Idle/Charging
        mockRobot.state.state = 8; // Charging
        mockRobot.state.error_code = 0;
        mockRobot.state.in_cleaning = 0;
        await vacuumFeatures.updateStatus();
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.state`]).to.equal(8);
        (0, vitest_1.expect)(mockAdapter.states[`Devices.${mockRobot.duid}.deviceStatus.error_code`]).to.equal(0);
