...
(0, vitest_1.describe)("Queue Deep Dive (requestsHandler)", () => {
    let mockAdapter;
    let handler;
    (0, vitest_1.beforeEach)(() => {
        ...
        mockAdapter.local_api = { isConnected: () => true, sendMessage: () => true, clearLocalDevicedTimeout: () => { } };
    ...
    (0, vitest_1.it)("should respect concurrency limit (10)", async () => {
        const duid = "duid";
        ...
        // Concurrency is 10.
        (0, vitest_1.expect)(maxActive).to.be.at.most(11); // Allow 1 buffer for timing
        (0, vitest_1.expect)(manager.queue.pending).to.be.above(0); // Should have items waiting
    });
    (0, vitest_1.it)("should prioritize high-priority requests", async () => {
        ...
        (0, vitest_1.expect)(executionOrder).to.deep.equal(["blocker", "high", "low"]);
    });
    (0, vitest_1.it)("does not reuse request IDs as local TCP transport message IDs", async () => {
        let payloadRequestId;
        let transportSequenceId;
        ...
        await new Promise(res => setTimeout(res, 10));
        (0, vitest_1.expect)(payloadRequestId).to.equal(301);
        (0, vitest_1.expect)(transportSequenceId).to.equal(1);
        handler.resolvePendingRequest(301, ["ok"], 4, "duid", "TCP");
        await (0, vitest_1.expect)(requestPromise).resolves.to.deep.equal(["ok"]);
    });
});
