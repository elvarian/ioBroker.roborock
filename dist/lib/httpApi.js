...
class http_api {
    ...
    async initializeRealApi() {
        ...
        try {
            ...
            const realApi = axios_1.default.create({
    ...
    async updateHomeData() {
        ...
        if (this.homeID) {
            try {
                const res = await this.realApi.get(`v3/user/homes/${this.homeID}`);
     ...
     */
    async getSharedDeviceRooms(duid) {
        if (!duid || !this.isSharedDevice(duid))
            return [];
        try {
            if (!this.realApi) {
            ...
            const res = await this.realApi.get(`user/deviceshare/query/${duid}/rooms`);
            const result = res.data?.result;
